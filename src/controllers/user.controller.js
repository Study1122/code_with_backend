...
const registerUser = asyncHandler(async (req, res, next) => {
    ...
    //console.log(avatarLocalPath, coverPhotoLocalPath);
    if (!avatarLocalPath){
    ...
    let coverPhotoUploadResponse = null;
    if (coverPhotoLocalPath){
        coverPhotoUploadResponse = await uploadOnCloudinary(coverPhotoLocalPath);
    }
    if ((!avatarUploadResponse) || (coverPhotoLocalPath && !coverPhotoUploadResponse)){
        throw new ApiErrors(500, "Image upload failed, please try again later");
    }
    ...
    const newUser = await new User.create({
        ...
        coverImage: coverPhotoUploadResponse?.url || "",
    });
    ...
    if (!createdUser){
        throw new ApiErrors(500, "User registration failed, Please try again later");
    }
